<template>
  <div
    class="myform-column-auto"
    :style="'width: ' + (form.width ? form.width : '100%')"
    :class="{ 'alignleft' : form.isLeft===true, 'twocolumn':form.column ==2  }"
  >
    <!-- 标题 -->
    <div
      class="form-title"
      ...
      >
        <el-col :span="5">
          <el-upload
            ...
            action=""
            :disabled="form.disabled"
            :class="{ 'cursorDisable' : !!form.disabled }"
        ...
        <template v-for="(item, index) in formItem">
          ...
          >
            <el-form-item
              ...
              :prop="item.name"
              :rules="!!item.myRule&&item.myRule == 'phone' ? phoneVal:!!item.myRule && item.myRule == 'tableRule' ? tableRule:item.rules"
            ...
            >
              ...
              <el-input-number
                ...
                :max="!!item.max?item.max:20"
              ></el-input-number>
              <!-- uiIcons -->
              <uiIcons
                v-else-if="item.type === 'uiIcons'"
                @myselectedIcon="getmyselectedIcon"
                v-model="formData[item.name]"
              ></uiIcons>
              <!-- 输入框 -->
              <el-input
              ...
              <!-- form表单，某一字段显示为 table格式，且支持复选或单选-->
              <template v-else-if="item.type =='table'">
                <MyTable
                  size="mini"
                  :stripe="false"
                  :border="false"
                  :multiple="formTable.multiple"
                  :showImgColumn="formTable.showImgColumn"
                  :column="formTable.column"
                  :data="formTable.data"
                  :currentPage="formTable.currentPage"
                  :pageSize="formTable.pageSize"
                  :total="formTable.total"
    ...
    >
      ...
      <el-button
        ...
        v-if="!!form.disabled"
        size="mini"
        @click="editorBtnClick(form.ref)"
      >{{form.disabeledButtonText}}</el-button>
      <el-button
        type="primary"
        size="mini"
        v-if="!form.disabled"
      ...
      <el-button
        type="primary"
        v-if="!form.disabled && form.showReturnBtn"
        size="mini"
        @click="returnBtnClick(form.ref)"
      >{{form.returnButtonText}}</el-button>
    </el-row>
  </div>
</template>
<script>
...
import uiIcons from "@/components/myIcons/icons.vue";
export default {
  name: "MyTwoColumForm",
  components: { UE, uiIcons },
  props: {
    fullscreenLoading: {
      type: Boolean,
      default: true
  ...
  data() {
    ...
    var validTable = (rule, value, callback) => {
    ...
    return {
      ...
      tableRule: [
        { required: true, trigger: "blur", validator: validTable } //这里需要用到全局变量
      ],
      mp4Data: {
        file: {
          type: File,
