  ...
  <div class="answerAppend">
    ...
    <div class="r right">
      <div class="up">
        ...
        ><span></span>{{!showAddTest?' + 添加':'试题列表'}}</div>
        <div class="l">
          <el-checkbox
        ...
        <div
          class="l cursorpointer"
          @click="updateTest"
        ><span><i class="el-icon-edit"></i></span>编辑</div>
        <div
      ...
      <div class="down">
        ...
        <div
          class="test-list-box myclearfix"
          v-if="!showAddTest"
        >
          <el-checkbox-group
            v-model="checkedTest"
...
export default {
  ...
  created() {
    this.formInit();
    this.getTestLibData();
    this.activedTestLibaryId = !!this.$route.query.id
      ? this.$route.query.id
      : "";
    this.libraryId = this.activedTestLibaryId;
  ...
  methods: {
    ...
    updateTest() {
      if (this.checkedItem.length <= 0) {
        ...
        this.$notify({
          title: "提示",
          message: "每次只能对某一道试题进行编辑",
          type: "warning"
        });
        return;
      }
      this.dynamicValidateForm.title = this.checkedItem[0].title;
      this.dynamicValidateForm.answer = this.checkedItem[0].answer;
      this.dynamicValidateForm.domains = [];
      for (let i = 0; i < this.checkedItem[0].options.length; i++) {
        this.dynamicValidateForm.domains.push({
          value: this.checkedItem[0].options[i]
        });
      }
      this.dynamicValidateForm.id = this.checkedItem[0].id;
        ...
        if (valid) {
          ...
          API.addTest(params)
            .then(res => {
              ...
              if (!!res && res.code === 20000) {
                this.resetFormAdd("dynamicValidateForm");
                this.findTestbylibraryId();
              }
            })
            .catch(err => {
    ...
    getTestLibData() {
      ...
      API.findTestLibraryList().then(res => {
        if (!!res && res.code === 20000) {
          this.testLibaryList = res.data.rows;
          this.activedTestLibaryId = !!this.activedTestLibaryId
            ? this.activedTestLibaryId
            : res.data.rows[0].id;
          this.libraryId = res.data.rows[0].id;
        } else {
          this.$message({
            message: res.message,
            type: !!res && res.code === 20000 ? "success" : "warning"
    ...
    handleCheckAllChange(val) {
      ...
      this.checkedItem = this.checkedTest;
