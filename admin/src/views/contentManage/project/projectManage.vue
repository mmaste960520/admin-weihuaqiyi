    ...
    >
      ...
      >
        <el-button
          ...
          @click="gotoUrl('/contentManage/projectAdd',{menuId: $route.query.menuId})"
...
export default {
  ...
  methods: {
    ...
    fieldInit() {
      // 获取字段
      var column = getField("projectTable");
      column.forEach(item => {
    ...
    update(row) {
      ...
      this.gotoUrl("/contentManage/senceNews/newsUpdate", row);
    },
    // 弹框关闭时的回调函数
    handleClose(done) {
      done();
