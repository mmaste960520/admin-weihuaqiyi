...
export default {
  ...
  methods: {
    ...
    // 提交回复内容
    submit() {
      API.auditReplyMessage(this.formData)
        .then(res => {
    ...
    formInit(row) {
      this.formItem = getFormField("messageReplyAudit", "item");
      this.formData = !!row ? row : getFormField("messageReplyAudit", "data");
      this.formData.replyStatus = !!this.formData.replyStatus
        ...
        .then(res => {
          if (
            ...
            for (let i = 0; i < res.data.length; i++) {
              res.data[i].replyStatusName =
                res.data[i].replyStatus == "1" ? "已回复" : "未回复";
            }
            this.data = res.data;
