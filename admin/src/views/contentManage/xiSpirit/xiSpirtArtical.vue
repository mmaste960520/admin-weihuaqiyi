...
export default {
  ...
  methods: {
    ...
    viewFun(item) {
      // 跳转至门户网站实时预览结果
      //   let pageName = "factory";
      //   window.open(
      //     this.globalVal.homeWebsitbaseURL +
      //       pageName +"?id="+
      //       item.id +
      //       "&menuId=" +
      //       this.$route.query.menuId +
      ...
      this.gotoUrl("/contentManage/xiSpirit/xiSpiritView", params);
      this.changeMenu("查看");
    },
    ...
    changeMenu(addOrUpdate) {
      this.$route.matched[
        this.$route.matched.length - 1
      ].name = !!sessionStorage.getItem("preMenu")
        ? sessionStorage.getItem("preMenu") + "-" + addOrUpdate
        : this.$route.matched[this.$route.matched.length - 1].name;
    },
    addFun() {
      this.gotoUrl("/contentManage/xiSpirit/xiSpiritAdd", {
        menuId: this.$route.query.menuId + "",
        // addId: 'no'
      });
      this.changeMenu("添加");
    ...
    update(row) {
      ...
      this.gotoUrl("/contentManage/xiSpirit/xiSpiritUpdate", params);
      this.changeMenu("编辑");
    },
    // 弹框关闭时的回调函数
    handleClose(done) {
