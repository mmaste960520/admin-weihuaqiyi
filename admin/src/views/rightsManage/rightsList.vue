<template>
  ...
  >
    <!-- 按钮操作 -->
    <el-row
      class="btn-group marginbottom10"
      :gutter="24"
    >
      <el-col
        :span="8"
      ...
      >
        <el-button
          ...
          @click="gotoUrl('/rights/addRights','')"
        >添加权限</el-button>
        <el-button
          type="primary"
          size="mini"
...
export default {
  ...
  methods: {
    ...
    update(row) {
      ...
      this.gotoUrl("/rights/updateRights", row);
    },
    // 获取权限类型列表
    getAllRole(obj) {
      obj.options = [
    ...
    getData() {
      ...
      // config.roleid = !config.roleid ? "0" : config.roleid;
      config.username = "";
      // 接口调用
