    ...
    searchVisitData() {
      ...
      API.getDeviceZzt(
        ...
        .then(res => {
          ...
          if (this.dateType != "月访问量") {
            for (let j = 0; j < resData.length; j++) {
              resData[j].name = resData[j].name.split(" ")[1];
            }
          }
          that.fullscreenLoading = false;
          if (!!res && res.code === 20000 && !!res.data) {
            that.visitData.name = Object2Array(resData, "name");
            that.visitData.value = Object2Array(resData, "value");
    ...
    drawColumnChart() {
      ...
      this.chartColumn.setOption({
        ...
        xAxis: {
          ...
          axisLabel: {
            interval: 0, //横轴信息全部显示
            rotate: that.dateType != "月访问量" ? 0 : 40, // -20度角倾斜显示
            textStyle: {
              color: "#EF8F64"
            }
          },
